#!/bin/sh
# Builds the Snes9x waterbox core package and installs it into a chimera
# checkout as build/Cores/snes9x.chimeraCore.
#
...
s9x_pin="$(git -C "$root/extern/snes9x" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "upstream": {"name": "Snes9x", "pin": "$s9x_pin"},
    "builtOn": "$os_id",
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/snes9x.chimeraCore"
rm -f "$zip_path"
...
for cache in "$chimera_root"/build/CoreCache/snes9x-*; do
