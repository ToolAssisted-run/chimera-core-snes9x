...
if len(sys.argv) > 4:
    ...
    for decl_id, path in json.loads(sys.argv[4]).items():
        fw["Snes9x/" + decl_id] = path
# Pick this core explicitly: an install can hold several packages claiming the SNES system, and --core only LOADS a package, it does not choose it.
cfg.setdefault("DefaultCores", {})["SNES"] = "Snes9x"
